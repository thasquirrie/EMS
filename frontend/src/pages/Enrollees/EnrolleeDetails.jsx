...
import {
  ...
  editEnrollee,
  getEnrolleeDetails,
} from '../../actions/enrolleeActions';
import Modal from '../../components/Modal';
import { EDIT_ENROLLEE_RESET } from '../../constants/enrolleesConstants';

// console.log(NaijaStates);

const AddEnrollee = ({ match, history }) => {
  const dispatch = useDispatch();
  // console.log({ match });
  const enrolleeId = match.params.id;
  ...
  console.log(enrolleeDetail.nin);

  // console.log(enrolleeId);

  const enrolleeDetails = useSelector((state) => state.enrolleeDetails);
  const { loading, enrollee } = enrolleeDetails;
  // console.log({ enrollee });

  const enrolleeEdit = useSelector((state) => state.enrolleeEdit);
  const {
    // loading: editLoading,
    success: editSuccess,
    error: editError,
    // enrollee: editedEnrollee,
  } = enrolleeEdit;

  useEffect(() => {
    if (!enrollee.firstName || enrollee.id !== enrolleeId) {
      console.log('This was called');
      dispatch(getEnrolleeDetails(enrolleeId));
    } else {
      setFirstName(enrollee.firstName);
      setLastName(enrollee.lastName);
      setMiddleName(enrollee.middleName);
      setEmail(enrollee.email);
      setTrackingID(enrollee.trackingID);
      setPhone(enrollee.phone);
      setAssociatedTrackingID(enrollee.associatedTrackingID);
      setBvn(enrollee.bvn);
      setTypeOfTransaction(enrollee.typeOfTransaction);
      setModeOfTransaction(enrollee.modeOfTransaction);
      setGender(enrollee.gender);
      setChannel(enrollee.channel);
      setPlasticIDCard(enrollee.plasticIDCard);
      setAddress(enrollee.address);
      setAddressState(enrollee.addressState);
      setAddressCity(enrollee.addressCity);
      setLga(enrollee.lga);
      setStateOfOrigin(enrollee.stateOfOrigin);
      setlgaOfOrigin(enrollee.lgaOfOrigin);
      setCountry(enrollee.country);
      setNin(enrollee.nin);

      if (editSuccess) {
        dispatch({ type: EDIT_ENROLLEE_RESET });
        history.push('/enrollees');
      }
    }
  }, [enrolleeId, dispatch, enrollee, editSuccess, history]);

  const submitHandler = (e) => {
    e.preventDefault();
    console.log({ enrolleeDetails });

    dispatch(editEnrollee(enrolleeId, enrolleeDetail));
    // dispatch({
    //   type: EDIT_ENROLLEE_RESET,
    // });
  };

  // console.log(enrollee.address);
  // console.log(enrollee.lga);
  console.log({ editError, editSuccess });
  ...
  return (
    <>
      ...
      {editError && (
        <Modal type='error' title='Update Error' message={editError} />
      )}
